{{--
    Filter Partial for Purchase Request Release List
    ...
    <div class="row">
        <div class="col-12">
            <div class="d-flex justify-content-center gap-3 mt-4">
                <button type="button" class="btn btn-primary" onclick="releaseListManager.searchRelease()">
                    <i class="icon-base bx bx-search me-2"></i>
                    Search
                </button>
                <button type="button" class="btn btn-secondary" onclick="releaseListManager.resetFilter()">
...
<script>
    (function() {
        ...
        async function loadCompanies() {
            ...
            try {
                ...
                renderDropdownWithSearch({
                    ...
                    limit: 5,
                    onItemSelect: (item, value, text) => {
                        // Store Type/Sub Type ID in hidden field
                        const hiddenInput = document.getElementById(item.ID ? 'prType' : 'prSubType');
                        const itemId = item.ID || item.id || '';
                        if (hiddenInput && itemId) {
                            hiddenInput.value = itemId.toString();
                        }
                    }
                });
        ...
        // Reset Filter function for releaseListManager
        if (typeof window.releaseListManager === 'undefined') {
            window.releaseListManager = {};
        }

        // Only add resetFilter if it doesn't exist or if releaseListManager is not a manager instance
        if (!window.releaseListManager.resetFilter || typeof window.releaseListManager.resetFilter !== 'function') {
            window.releaseListManager.resetFilter = function() {
