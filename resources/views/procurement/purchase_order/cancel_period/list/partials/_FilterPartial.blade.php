    ...
    <div class="row mt-3">
        <div class="col-12">
            <div class="d-flex justify-content-center gap-3">
                <button type="button" class="btn btn-primary" onclick="cancelPeriodManager.searchPO()">
                    <i class="icon-base bx bx-search me-2"></i>
                    Search
                </button>
                <button type="button" class="btn btn-secondary" onclick="cancelPeriodManager.resetFilter()">
    ...
    (function() {
        ...
        async function loadPurchaseTypes() {
            ...
            try {
                ...
                if (typeSearchInput && typeSearchInput.parentNode) {
        ...
        async function loadPurchaseSubTypes(typeId) {
            ...
            try {
                ...
                if (window.procurementSharedCache && typeof window.procurementSharedCache.getPurchaseSubTypes === 'function') {
                    ...
                    const endpoint = `/Procurement/Master/PurchaseSubTypes/List?PurchaseTypeID=${encodeURIComponent(typeId)}&isActive=true`;
                    const data = await apiCall('Procurement', endpoint, 'GET', null, { cancelPrevious: false })
                        .catch((error) => {
                            if (String(error?.message || '').toLowerCase().includes('cancel')) {
                                return null;
                ...
                if (subTypeSearchInput && subTypeSearchInput.parentNode) {
        ...
        async function loadCompanies() {
            ...
            try {
                let companies = [];
                if (window.procurementSharedCache && typeof window.procurementSharedCache.getCompanies === 'function') {
                ...
                renderDropdownWithSearchSafe({
                    ...
                    getValue: (company) => (company.ID || company.id || '').toString(),
        ...
        async function loadVendors() {
            ...
            try {
                let vendors = [];
                if (window.procurementSharedCache && typeof window.procurementSharedCache.getVendors === 'function') {
        ...
        window.loadPurchaseSubTypes = loadPurchaseSubTypes;
    })();
</script>
