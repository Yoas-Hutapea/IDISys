/**
 * ApprovalPOListAPI Module
 * Handles all API calls for Approval PO List with caching to prevent duplicate API calls
 */
class ApprovalPOListAPI {
    constructor() {
        // Use shared cache if available, otherwise create local cache
        this.sharedCache = window.procurementSharedCache || null;
        
        // Cache for API responses (for Approval PO-specific data)
        this.cache = new Map();
     ...
     */
    async submitBulkApproval(poNumbers, statusID, decision, remarks, activity) {
        const endpoint = `/Procurement/PurchaseOrder/Approval/Bulk`;
        const payload = {
            poNumbers: poNumbers,
            mstApprovalStatusID: statusID, // 10 for Approve, 11 for Reject
            Decision: decision,
            Remark: remarks,
            Activity: activity
        };
     ...
     */
    async submitApproval(poNumber, decision, remarks, activity) {
        const endpoint = `/Procurement/PurchaseOrder/Approval/${encodeURIComponent(poNumber)}`;
        const payload = {
            Decision: decision,
            Remark: remarks.trim(),
            Activity: activity
...
if (typeof window !== 'undefined') {
    window.ApprovalPOListAPI = ApprovalPOListAPI;
}
