/**
 * ReleaseListFilter Module
 * Handles filter functionality, search, and reset
 */
class ReleaseListFilter {
    ...
    debouncedSearch() {
        ...
        this.debounceTimer = setTimeout(() => {
            ...
            if (this.manager && this.manager.searchRelease) {
                this.manager.searchRelease();
            }
        }, 500);
    }
    ...
    buildFilter() {
        ...
        if (filters.company && filters.company.trim()) filter.source = filters.company.trim().toUpperCase();
        if (filters.regional && filters.regional.trim()) filter.regional = filters.regional.trim();
        if (filters.typeSPK && filters.typeSPK.trim()) filter.typeSPK = filters.typeSPK.trim();

        return filter;
    }

    /**
     * Build filter for bulky release table
     */
    buildBulkyFilter() {
        const form = document.getElementById('bulkyReleaseFilterForm');
        if (!form) return {};
        ...
        const regional = formData.get('regional');
        const prNumber = formData.get('prNumber');
        const typeSPK = formData.get('typeSPK');

        if (startDate) filter.fromDate = startDate;
        if (endDate) filter.toDate = endDate;
        if (year) filter.year = year;
        if (prNumber && prNumber.trim()) filter.purchReqNum = prNumber.trim();
        if (prType && prType.trim()) filter.purchReqType = prType.trim();
        if (prName && prName.trim()) filter.purchReqName = prName.trim();
        if (company && company.trim()) filter.source = company.trim().toUpperCase();
        if (regional && regional.trim()) filter.regional = regional.trim();
        if (typeSPK && typeSPK.trim()) filter.typeSPK = typeSPK.trim();
    ...
    async resetFilter() {
        ...
        if (this.manager && this.manager.tableModule && this.manager.tableModule.searchRelease) {
            await this.manager.tableModule.searchRelease();
    ...
    async resetBulkyReleaseFilter() {
        ...
        // Clear selected PRs and reset mode
        if (this.manager && this.manager.selectedPRNumbers) {
            this.manager.selectedPRNumbers.clear();
        }
        if (this.manager && this.manager.bulkyCheckboxMode !== undefined) {
        ...
        if (this.manager && this.manager.tableModule && this.manager.tableModule.updateBulkySelectAllCheckbox) {
            this.manager.tableModule.updateBulkySelectAllCheckbox();
        }
        
        // Reset bulky release table
        if (this.manager && this.manager.tableModule && this.manager.tableModule.bulkyReleaseDataTable) {
            this.manager.tableModule.bulkyReleaseDataTable.ajax.reload();
        }
    }
...
if (typeof window !== 'undefined') {
    window.ReleaseListFilter = ReleaseListFilter;
}
