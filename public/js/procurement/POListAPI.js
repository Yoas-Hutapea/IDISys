/**
 * POListAPI Module
 * Handles all API calls for PO List with caching to prevent duplicate API calls
 */
class POListAPI {
    constructor() {
        ...
        // Cache for API responses (for PO-specific data)
        this.cache = new Map();
    ...
    /**
     * Get approval history by PO Number (with shared caching)
     */
    async getApprovalHistory(poNumber) {
        // Use shared cache if available
        if (this.sharedCache && this.sharedCache.getCachedData) {
            const cacheKey = `poApprovalHistory_${poNumber}`;
            return await this.sharedCache.getCachedData(cacheKey, async () => {
                const response = await apiCall('Procurement', `/Procurement/PurchaseOrder/Approval/History/${encodeURIComponent(poNumber)}`, 'GET');
    ...
    clearPOCache(poNumber) {
        ...
        if (this.sharedCache && this.sharedCache.clearCacheEntry) {
            ...
            this.sharedCache.clearCacheEntry(`poApprovalHistory_${poNumber}`);
        }
    }
