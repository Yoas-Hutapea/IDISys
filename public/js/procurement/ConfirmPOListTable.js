...
class ConfirmPOListTable {
    ...
    async initializeDataTable() {
        ...
        this.dataTable = CreateTable({
            ...
            tableId: '#confirmPOTable',
            columns: ColumnBuilder([
                {
                    // Checkbox column for bulk confirm
                    data: 'purchOrderID',
                    title: '',
                    orderable: false,
                    searchable: false,
                    ...
                    render: (data, type, row) => {
                        ...
                        return `<input type="checkbox" class="row-checkbox" data-po-number="${self.escapeHtml(poNumber)}" ${isSelected ? 'checked' : ''} onchange="confirmPOManager.toggleSelectPO('${self.escapeHtml(poNumber)}')">`;
                    }
                },
                {
                    type: 'actions',
        ...
        // Bind action buttons
        $(document).off('click', '#confirmPOTable .action-btn').on('click', '#confirmPOTable .action-btn', function() {
            ...
            switch(action) {
                case 'view':
                    if (self.manager && self.manager.viewModule && self.manager.viewModule.viewConfirm) {
                        self.manager.viewModule.viewConfirm(poNumber);
                    }
                    break;
            }
        });
        ...
        if (this.manager) {
            this.manager.confirmPODataTable = this.dataTable;
        }
    }
...
if (typeof window !== 'undefined') {
    window.ConfirmPOListTable = ConfirmPOListTable;
}
