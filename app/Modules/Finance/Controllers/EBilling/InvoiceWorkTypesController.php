...
class InvoiceWorkTypesController extends Controller
{
    public function index(Request $request)
    {
        $table = $this->resolveTable([
            'mstFINInvoiceWorkType',
            'mstFINInvoiceWorkTypes',
        ]);
    ...
    public function show(int $id)
    {
        ...
        $idColumn = $this->resolveColumn($table, ['ID', 'Id', 'InvoiceWorkTypeID', 'mstFINInvoiceWorkTypeID']);
        if (!$idColumn) {
            return response()->json(null, 404);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (!$table) {
            return response()->json(['message' => 'Invoice work type table not found'], 404);
        }

        $data = [];
        $this->setIfColumn($table, $data, ['WorkType', 'workType'], $request->input('workType'));
        $this->setIfColumn($table, $data, ['WorkTypeDescription', 'workTypeDescription'], $request->input('workTypeDescription'));
        $this->setIfColumn($table, $data, ['COA', 'coa'], $request->input('coa'));
    ...
    public function update(int $id, Request $request)
    {
        ...
        $idColumn = $this->resolveColumn($table, ['ID', 'Id', 'InvoiceWorkTypeID', 'mstFINInvoiceWorkTypeID']);
        if (!$idColumn) {
            return response()->json(['message' => 'Invoice work type ID column not found'], 404);
        }
