...
class InvoiceWorkTypePurchaseTypesController extends Controller
{
    ...
    public function show(int $id)
    {
        ...
        $idColumn = $this->resolveColumn($table, ['ID', 'Id', 'InvoiceWorkTypePurchaseTypeID', 'mstFINInvoiceWorkTypePurchaseTypeID']);
        if (!$idColumn) {
            return response()->json(null, 404);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (!$table) {
            return response()->json(['message' => 'Invoice work type purchase type table not found'], 404);
        }

        $data = [];
        $this->setIfColumn($table, $data, ['InvoiceWorkTypeID', 'mstFINInvoiceWorkTypeID'], $request->input('invoiceWorkTypeID'));
        $this->setIfColumn($table, $data, ['PurchaseTypeID', 'mstPROPurchaseTypeID'], $request->input('purchaseTypeID'));
        $this->setIfColumn($table, $data, ['PurchaseSubTypeID', 'mstPROPurchaseSubTypeID'], $request->input('purchaseSubTypeID'));
        $this->setIfColumn($table, $data, ['Description', 'description'], $request->input('description'));
    ...
    public function update(int $id, Request $request)
    {
        ...
        $idColumn = $this->resolveColumn($table, ['ID', 'Id', 'InvoiceWorkTypePurchaseTypeID', 'mstFINInvoiceWorkTypePurchaseTypeID']);
        if (!$idColumn) {
            return response()->json(['message' => 'Invoice work type purchase type ID column not found'], 404);
        }
