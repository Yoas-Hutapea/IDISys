...
class InvoiceWorkTypeTOPDocumentChecklistsController extends Controller
{
    public function index(Request $request)
    {
        $table = $this->resolveTable([
            'mstFINInvoiceWorkTypeTOPDocumentChecklist',
            'mstFINInvoiceWorkTypeTOPDocumentChecklists',
        ]);
    ...
    public function show(int $id)
    {
        ...
        $idColumn = $this->resolveColumn($table, ['ID', 'Id', 'InvoiceWorkTypeTOPDocumentChecklistID', 'mstFINInvoiceWorkTypeTOPDocumentChecklistID']);
        if (!$idColumn) {
            return response()->json(null, 404);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (!$table) {
            return response()->json(['message' => 'Invoice work type TOP document checklist table not found'], 404);
        }

        $data = [];
        $this->setIfColumn($table, $data, ['InvoiceWorkTypeID', 'mstFINInvoiceWorkTypeID'], $request->input('invoiceWorkTypeID'));
        $this->setIfColumn($table, $data, ['TermOfPaymentID', 'trxFINInvoiceTOPID', 'mstFINInvoiceTOPID'], $request->input('termOfPaymentID'));
        $this->setIfColumn($table, $data, ['TermNumber', 'PeriodNumber', 'TermValue'], $request->input('termNumber'));
        $this->setIfColumn($table, $data, ['DocumentChecklistID', 'mstFINInvoiceDocumentChecklistID'], $request->input('documentChecklistID'));
        $this->setIfColumn($table, $data, ['IsMandatory', 'isMandatory'], $this->normalizeBool($request->input('isMandatory')));
        $this->setIfColumn($table, $data, ['IsAuto', 'isAuto'], $this->normalizeBool($request->input('isAuto')));
    ...
    public function update(int $id, Request $request)
    {
        ...
        $idColumn = $this->resolveColumn($table, ['ID', 'Id', 'InvoiceWorkTypeTOPDocumentChecklistID', 'mstFINInvoiceWorkTypeTOPDocumentChecklistID']);
        if (!$idColumn) {
            return response()->json(['message' => 'Invoice work type TOP document checklist ID column not found'], 404);
        }
